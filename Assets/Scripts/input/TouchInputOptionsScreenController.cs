...
public class TouchInputOptionsScreenController : MonoBehaviour
{
    ...
    [SerializeField]
    GameObject OptionsManagerObject;
    ...
    private void initializeOptionScreenTouchControls()
    {
        ...
        {
            OptionsManagerObject = FindObjectOfType<OptionsManager>().gameObject;
            //Fetch the Raycaster from the GameObject (the Canvas)
            m_Raycaster = FindObjectOfType<OptionsManager>().gameObject.GetComponentInChildren<GraphicRaycaster>();
            //Fetch the Event System from the Scene
            m_EventSystem = FindObjectOfType<OptionsManager>().gameObject.GetComponentInChildren<EventSystem>();
        }
    ...
    private void activateDoubleTappedButton()
    {
        ...
        //// create new
        //if (EventSystem.current.currentSelectedGameObject.name.Equals(OptionsManager.CreateNewButtonName))
        //{
        //    SceneManager.LoadSceneAsync(Constants.SCENE_NAME_level_00_account_createNew);
        //}
        //// login existing
        //if (EventSystem.current.currentSelectedGameObject.name.Equals(OptionsManager.LoginExistingButtonName))
        //{
        //    SceneManager.LoadSceneAsync(Constants.SCENE_NAME_level_00_account_loginExisting);
        //}
        //// login local
        //if (EventSystem.current.currentSelectedGameObject.name.Equals(OptionsManager.LoginLocalButtonName))
        //{
